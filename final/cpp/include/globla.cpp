...
cv::Mat downsample(cv::Mat const &img, int const &factor) {
    if (img.type() != CV_8UC3 && img.type() != CV_32SC1) {
        eprintf("Unhandled image type %d (expected CV_8UC3 or CV_32SC1)\n",
                img.type());
    }
    ...
    for (int y = 0; y < rows; ++y) {
        for (int x = 0; x < cols; ++x) {
            ...
            } else if (ret.type() == CV_32SC1) {
                ret.at<int>(y, x) = img.at<int>(y * factor, x * factor);
            } else {
                eprintf("Unhandled image type %d (expected CV_8UC3 or "
                        "CV_32SC1)\n",
                        ret.type());
            }
        }
...
cv::Mat upsample(cv::Mat const &img, int const &factor) {
    ...
    if (ret.type() == CV_8UC3) {
                ...
                if (y / factor + 1 == img.rows ||
                    ...
                    cv::Vec3b hor_0 =
                        lerp(img.at<cv::Vec3b>(y / factor, x / factor),
                             ...
                             1.0 * xoffset / factor);
                    cv::Vec3b hor_1 = lerp(
                        img.at<cv::Vec3b>(y / factor + 1, x / factor),
        ...
        for (int y = 0; y < rows; ++y) {
            ...
            for (int x = 0; x < cols; ++x) {
                int  xoffset = x % factor;
                int &col     = ret.at<int>(y, x);
                if (y / factor + 1 == img.rows ||
                    x / factor + 1 == img.cols) {
                    col = img.at<int>(y / factor, x / factor);
                } else {
                    int hor_0 = lerp(img.at<int>(y / factor, x / factor),
                                     img.at<int>(y / factor, x / factor + 1),
                                     1.0 * xoffset / factor);
                    int hor_1 =
                        lerp(img.at<int>(y / factor + 1, x / factor),
                             img.at<int>(y / factor + 1, x / factor + 1),
                             1.0 * xoffset / factor);
                    col = lerp(hor_0, hor_1, 1.0 * yoffset / factor);
                }
            }
