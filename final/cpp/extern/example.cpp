...
//  is a set of sites (pixels) of width 10 and hight 5. Thus number of pixels
//  is 50 grid neighborhood: each pixel has its left, right, up, and bottom
//  pixels as neighbors 7 labels Data costs: D(pixel,label) = 0 if pixel < 25
//  and label = 0
...
// Below in the main program, we illustrate different ways of setting data and
// smoothness costs that our interface allow and solve this optimizaiton
// problem

// For most of the examples, we use no spatially varying pixel dependent
// terms. For some examples, to demonstrate spatially varying terms we use
...
//
void GridGraph_Individually(int width, int height, int num_pixels,
                            int num_labels) {
    ...
    try {
        GCoptimizationGridGraph *gc =
            new GCoptimizationGridGraph(width, height, num_labels);
        ...
        for (int l1 = 0; l1 < num_labels; l1++)
            for (int l2 = 0; l2 < num_labels; l2++) {
                int cost =
                    (l1 - l2) * (l1 - l2) <= 4 ? (l1 - l2) * (l1 - l2) : 4;
                gc->setSmoothCost(l1, l2, cost);
            }

        printf("\nBefore optimization energy is %d", gc->compute_energy());
        gc->expansion(2); // run expansion for 2 iterations. For swap use
...
// grid neighborhood structure is assumed
//
void GridGraph_DArraySArray(int width, int height, int num_pixels,
                            int num_labels) {
...
// V(p1,p2,l1,l2) = w_{p1,p2}*[min((l1-l2)*(l1-l2),4)], with
// w_{p1,p2} = p1+p2 if |p1-p2| == 1 and w_{p1,p2} = p1*p2 if |p1-p2| is not 1
void GridGraph_DArraySArraySpatVarying(int width, int height, int num_pixels,
                                       int num_labels) {
    int *result = new int[num_pixels]; // stores result of optimization
...
//
void GeneralGraph_DArraySArray(int width, int height, int num_pixels,
                               int num_labels) {
...
// Namely V(p1,p2,l1,l2) = w_{p1,p2}*[min((l1-l2)*(l1-l2),4)], with w_{p1,p2}
// = p1+p2 if |p1-p2| == 1 and w_{p1,p2} = p1*p2 if |p1-p2| is not 1

void GeneralGraph_DArraySArraySpatVarying(int width, int height,
                                          int num_pixels, int num_labels) {
    int *result = new int[num_pixels]; // stores result of optimization
...
int main(int argc, char **argv) {
    ...
    GeneralGraph_DArraySArraySpatVarying(width, height, num_pixels,
                                         num_labels);

    printf("\n  Finished %d (%d) clock per sec %d", clock() / CLOCKS_PER_SEC,
