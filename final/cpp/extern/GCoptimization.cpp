...
// Choose reasonably high-precision timer (sub-millisec resolution if
// possible).
#ifdef _WIN32
...
gcoclock() // TODO: not thread safe; separate begin/end so that end doesn't
           // have to check for query frequency
{
    gcoclock_t result = 0;
...
template <>
void GCoptimization::setupDataCostsExpansion<
    GCoptimization::DataCostFnSparse>(SiteID size, LabelID alpha_label,
...
void GCoptimization::specializeSmoothCostFunctor(const UserFunctor f) {
    ...
    m_setupSmoothCostsSwap =
        &GCoptimization::setupSmoothCostsSwap<UserFunctor>;
}
...
GCoptimization::EnergyType GCoptimization::giveSmoothEnergyInternal() {
    ...
    for (i = 0; i < m_num_sites; i++) {
        ...
        for (n = 0; n < numN; n++) {
            nSite = nPointer[n];
            if (nSite < i)
                eng += weights[n] * (sc->compute(i, nSite, m_labeling[i],
                                                 m_labeling[nSite]));
        }
    }
...
OLGA_INLINE void GCoptimization::addterm1_checked(EnergyT *e, VarID i,
    ...
    if (e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM)
        handleError("Data cost term was larger than GCO_MAX_ENERGYTERM; "
...
OLGA_INLINE void GCoptimization::addterm1_checked(EnergyT *e, VarID i,
                                                  ...
                                                  EnergyTermType w) {
    if (e0 > GCO_MAX_ENERGYTERM || e1 > GCO_MAX_ENERGYTERM)
        handleError("Smooth cost term was larger than GCO_MAX_ENERGYTERM; "
                    "danger of integer overflow.");
    if (w > GCO_MAX_ENERGYTERM)
        handleError("Smoothness weight was larger than GCO_MAX_ENERGYTERM; "
                    "danger of integer overflow.");
...
OLGA_INLINE void GCoptimization::addterm2_checked(
    ...
    if (e00 + e11 > e01 + e10)
        handleError("Non-submodular expansion term detected; smooth costs "
                    "must be a metric for expansion");
...
bool GCoptimization::solveSpecialCases(EnergyType &energy) {
    ...
    if (dc && !sc && !lc) {
        ...
        for (SiteID i = 0; i < m_num_sites; ++i) {
            ...
            if (minCostLabel > GCO_MAX_ENERGYTERM)
                handleError("Data cost was larger than GCO_MAX_ENERGYTERM; "
                            "danger of integer overflow.");
    ...
    if (!dc && !sc && lc) {
        ...
        LabelID    minLabel = 0;
        EnergyType minLabelCost =
            GCO_MAX_ENERGYTERM * (EnergyType)m_num_labels;
...
class GCoptimization::GreedyIter<GCoptimization::DataCostFnSparse> {
  public:
    ...
    OLGA_INLINE GreedyIter &operator++() {
        ...
        // are stored as consecutive [sparse] SiteIDs with respect to each
        // label.
        if (++m_site == m_siteend) {
...
GCoptimization::EnergyType GCoptimization::solveGreedy() {
    ...
    try {
        ...
        for (LabelID l = 0; l < m_num_labels; ++l) {
            ...
            for (; !iter.done(); ++iter) {
                ...
                if (dataCost > GCO_MAX_ENERGYTERM)
                    handleError(
                        "Data cost was larger than GCO_MAX_ENERGYTERM; "
                        "danger of integer overflow.");
                e[l] += dataCost;
                if (e[l] >
                    e[alpha]) // break out early if this will definitely
        ...
        // List of labels in the order that they were expanded upon (order[0]
        // first, order[1] second, ...)
        for (LabelID l = 0; l < m_num_labels; ++l)
            order[l] = l;
        order[alpha] = 0;
        order[0]     = alpha;
        ...
        for (LabelID alpha_count = 1; alpha_count <= m_num_labels;
            ...
            // Loop over all sites and all remaining labels to calculate
            // energy drop.
        ...
        efinal = e[alpha];
        if (efinal < estart) {
            // Greedy succeeded in lowering energy compared to initial
            // labeling
            delete[] oldLabeling;
            m_labelingInfoDirty = true;
            updateLabelingInfo(
                true, false,
                false); // update m_labelCounts only; m_labelingDataCosts and
                        // active labelcosts should be up to date
...
void GCoptimization::setDataCost(SiteID s, LabelID l, EnergyTermType e) {
    if (!m_datacostIndividual) {
        EnergyTermType *table =
            new EnergyTermType[m_num_sites * m_num_labels];
...
void GCoptimization::setDataCostFunctor(DataCostFunctor *f) {
    ...
    m_setupDataCostsSwap =
        &GCoptimization::setupDataCostsSwap<DataCostFunctor>;
...
void GCoptimization::setDataCost(LabelID l, SparseDataCost *costs,
    ...
    if (!m_datacostFn)
        ...
        handleError("Cannot apply sparse data costs after dense data costs "
                    "have been used.");
    m_labelingInfoDirty  = true;
    DataCostFnSparse *dc = (DataCostFnSparse *)m_datacostFn;
...
void GCoptimization::setSmoothCost(LabelID l1, LabelID l2, EnergyTermType e) {
    if (!m_smoothcostIndividual) {
        EnergyTermType *table =
            new EnergyTermType[m_num_labels * m_num_labels];
        memset(table, 0,
               m_num_labels * m_num_labels * sizeof(EnergyTermType));
        specializeSmoothCostFunctor(
            SmoothCostFnFromArray(table, m_num_labels));
...
void GCoptimization::setLabelSubsetCost(LabelID *labels, LabelID numLabels,
    ...
    if (cost > GCO_MAX_ENERGYTERM)
        handleError("Label cost was larger than GCO_MAX_ENERGYTERM; danger "
                    "of integer overflow.");
    ...
    for (LabelCostIter *lci = m_labelcostsByLabel[labels[0]]; lci;
        ...
        if (numLabels == lci->node->numLabels) {
            if (!memcmp(labels, lci->node->labels,
                ...
                // This label subset already exists, so just update the cost
                // and return
                lci->node->cost = cost;
                return;
            }
...
GCoptimization::EnergyType GCoptimization::expansion(int max_num_iterations) {
    ...
    try {
        if (max_num_iterations == -1) {
            // Strategic expansion loop focuses on labels that successfuly
            // reduced the energy
            ...
            do {
                ...
                // Make a pass over the unchecked labels in the current queue,
                // i.e. m_labelTable[next..queueSize-1]
                ...
                do {
                    if (!alpha_expansion(m_labelTable[next]))
                        std::swap(m_labelTable[next],
                                  m_labelTable[--queueSize]); // don't put
                                                              // this label in
                                                              // a new queue
                    else
                        ++next; // keep this label for the next (smaller)
                                // queue
                    m_stepsThisCycle++;
                } while (next < queueSize);

                if (next == start) // No expansion was successful, so try more
                                   // labels from the previous queue
                {
                    next = queueSizes.back();
                    queueSizes.pop_back();
                } else if (queueSize <
                           queueSizes.back() /
                               2) // Some expansions were successful, so focus
                                  // on them in a new queue
                {
                    next = 0;
                    queueSizes.push_back(queueSize);
                } else
                    next = 0; // All expansions were successful, so do another
                              // complete sweep
    ...
    m_stepsThisCycle = m_stepsThisCycleTotal =
        0; // set so that alpha_expansion() knows it's no inside expansion()
...
GCoptimization::setupLabelCostsExpansion(SiteID size, LabelID alpha_label,
    ...
    // outside the activeSites, since they cannot be eliminated by the
    // expansion.
    if (m_queryActiveSitesExpansion ==
            (SiteID(GCoptimization::*)(LabelID, SiteID *)) &
        GCoptimization::queryActiveSitesExpansion<DataCostFnSparse>) {
        // For sparse data costs, things are more complicated, because we must
        // ensure that no label cost for a fixed (non-active) non-alpha label
        // is encoded in the graph.
        ...
        for (LabelID l = 0; l < m_num_labels; ++l) {
            if (m_activeLabelCounts[l] != m_labelCounts[l]) {
                for (LabelCostIter *lcj = m_labelcostsByLabel[l]; lcj;
                     lcj                = lcj->next)
    ...
    // Since we're explicitly omitting the alpha_label label costs from the
    // binary energy, calculate what it would have been, so that we can
    // potentially reject the expansion afterwards.
...
bool GCoptimization::alpha_expansion(LabelID alpha_label) {
    if (alpha_label < 0)
        return false; // label was disabled due to setLabelOrder on subset of
                      // labels
    ...
    if (m_stepsThisCycleTotal == 0)
        m_labelingInfoDirty =
            true; // if not inside expansion(), assume data cost function
                  // could have changed since last expansion
    updateLabelingInfo();
    ...
    try {
        ...
        // Initialise reverse-lookup so that non-active neighbours can be
        // identified while constructing the graph
        for (SiteID i = 0; i < size; i++)
            m_lookupSiteVar[activeSites[i]] = i;

        // Create binary variables for each remaining site, add the data
        // costs, and compute the smooth costs between variables.
        EnergyT e(size + m_labelcostCount, // poor guess at number of pairwise
                                           // terms needed :(
                  m_numNeighborsTotal +
        ...
        for (SiteID i = 0; i < size; i++)
            m_lookupSiteVar[activeSites[i]] =
                -1; // restore m_lookupSite to all -1s
...
void GCoptimization::alpha_beta_swap(LabelID alpha_label,
    ...
    assert(alpha_label >= 0 && alpha_label < m_num_labels &&
           beta_label >= 0 && beta_label < m_num_labels);
    if (m_labelcostsAll)
    ...
    try {
        ...
        for (SiteID i = 0; i < size; i++) {
            ...
            m_lookupSiteVar[activeSites[i]] =
                -1; // restore lookupSiteVar to all -1s
        }
        m_labelingInfoDirty = true;
    } catch (...) {
        delete[] activeSites;
...
GCoptimizationGridGraph::GCoptimizationGridGraph(SiteID width, SiteID height,
    ...
    setupNeighbData(m_height - 1, m_height, m_width - 1, m_width, 2,
                    indexesDR);
}
...
void GCoptimization::printStatus1(int cycle, bool isSwap, gcoclock_t ticks0) {
    ...
    if (m_verbosity == 1) {
        ...
        // printing itself takes time (esp in MATLAB) and makes time useless
        // at this level
...
void GCoptimization::printStatus2(int alpha, int beta, int numVars,
    ...
    if (m_verbosity < 2)
        return;
    int microsec =
        (int)(1000000 * (gcoclock() - ticks0) / GCO_CLOCKS_PER_SEC);
    if (beta >= 0)
...
void GCoptimization::DataCostFnSparse::set(LabelID               l,
    ...
    //
    // Scan the list of costs and remember pointers to delimit the 'buckets',
    // i.e. where ranges of SiteIDs lie along the array. Buckets can be empty
    // (begin == end).
    //
    ...
    for (int i = 0; i < m_buckets_per_label; ++i) {
        ...
        while (next < end && next->site < end_site) {
            if (next->site < 0 || next->site >= m_num_sites)
                throw GCException("Invalid site id given for sparse data "
                                  "cost; must be within range.");
            if (next->site <= prev_site)
                throw GCException("Sparse data costs must be sorted in "
                                  "increasing order of SiteID");
            prev_site = next->site;
            ++next;
        }
        b[i].end = next;
...
GCoptimization::DataCostFnSparse::search(DataCostBucket &b, SiteID s) {
    ...
    return GCO_MAX_ENERGYTERM; // the site belongs to this bucket but with no
                               // cost specified
}
