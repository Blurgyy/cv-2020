...
class Graph {
  public:
    ...
    // pointer to the function which will be called if an error occurs; an
    // error message is passed to this function. If this argument is omitted,
    // exit(1) will be called.
    //
    // IMPORTANT: It is possible to add more nodes to the graph than
    // node_num_max (and node_num_max can be zero). However, if the count is
    // exceeded, then the internal memory is reallocated (increased by 50%)
    // which is expensive. Also, temporarily the amount of allocated memory
    // would be more than twice than needed. Similarly for edges. If you wish
    // to avoid this overhead, you can download version 2.2, where nodes and
    // edges are stored in blocks.
    ...
    // and 'rev_cap'. IMPORTANT: see note about the constructor
    ...
    //
    // Occasionally there may be several minimum cuts. If a node can be
    // assigned to both the source and the sink, then default_segm is
    // returned.
  ...
  public:
    ...
    ////////////////////////////////////////////////////////////////////////////////
    // 2. Functions for getting pointers to arcs and for reading graph
    // structure. //
    //    NOTE: adding new arcs may invalidate these pointers (if reallocation
    //    // happens). So it's best not to add arcs while reading graph
    //    structure.   //
    ////////////////////////////////////////////////////////////////////////////////
    ...
    // returns residual capacity of SOURCE->i minus residual capacity of
    // i->SINK
    ...
    // segmentation label. Nodes which are not in the list are guaranteed to
    // keep their old segmentation label (SOURCE or SINK). Example usage:
    //
    ...
    //			{
    //				G::node_id i = *ptr; assert(i>=0 &&
    //i<nodeNum); 				g->remove_from_changed_list(i);
    ...
    //
    // NOTE:
    //  - If changed_list option is used, then reuse_trees must be used as
    //  well.
  ...
  private:
    ...
    struct node {
        ...
        int is_sink : 1; // flag showing whether the node is in the source or
                         // in the sink tree (if parent!=NULL)
        int is_marked : 1;          // set by mark_node()
        int is_in_changed_list : 1; // set by maxflow if

        tcaptype tr_cap; // if tr_cap > 0 then tr_cap is residual capacity of
                         // the arc SOURCE->node otherwise         -tr_cap is
                         // residual capacity of the arc node->SINK
    };
    ...
    arc *arcs, *arc_last, *arc_max; // arc_last = arcs+2*edge_num, arc_max =
                                    // arcs+2*edge_num_max;
...
template <typename captype, typename tcaptype, typename flowtype>
inline void Graph<captype, tcaptype, flowtype>::set_rcap(arc *   a,
                                                         captype rcap) {
    assert(a >= arcs && a < arc_last);
    a->r_cap = rcap;
}
