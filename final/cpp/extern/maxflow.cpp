...
void Graph<captype, tcaptype, flowtype>::augment(arc *middle_arc) {
    ...
    for (i = middle_arc->sister->head;; i = a->head) {
        ...
        if (!a->sister->r_cap) {
            set_orphan_front(
                i); // add i to the beginning of the adoption list
        }
    }
    i->tr_cap -= bottleneck;
    if (!i->tr_cap) {
        ...
        for (a0 = i->first; a0; a0 = a0->next) {
            ...
            if (!j->is_sink && (a = j->parent)) {
                ...
                if (a != TERMINAL && a != ORPHAN && a->head == i) {
                    set_orphan_rear(
                        j); // add j to the end of the adoption list
                }
            }
        }
    }
...
Graph<captype, tcaptype, flowtype>::maxflow(bool            reuse_trees,
    ...
    if (changed_list && !reuse_trees) {
        if (error_function)
            (*error_function)(
                "changed_list cannot be used without reuse_trees!");
        exit(1);
    }
            ...
            for (a = i->first; a; a = a->next)
                if (a->r_cap) {
                    ...
                    if (!j->parent) {
                        ...
                        /* heuristic - trying to make the distance from j to
                         * the source shorter */
                        j->parent = a->sister;
                        j->TS     = i->TS;
                        j->DIST   = i->DIST + 1;
...
template <typename captype, typename tcaptype, typename flowtype>
void Graph<captype, tcaptype, flowtype>::test_consistency(
    node *current_node) {
    node *i;
    arc * a;
    int   r;
...
void Graph<captype, tcaptype, flowtype>::Copy(
    ...
    if (arc_max < arcs + (g0->arc_last - g0->arcs)) {
        free(arcs);
        arcs = arc_last =
            (arc *)malloc((g0->arc_last - g0->arcs) * sizeof(arc));
    ...
    for (i = nodes; i < node_last; i++) {
        if (i->first)
            i->first = (arc *)((char *)arcs +
                               (((char *)i->first) - ((char *)g0->arcs)));
        if (i->parent && i->parent != TERMINAL && i->parent != ORPHAN)
            i->parent = (arc *)((char *)arcs +
                                (((char *)i->parent) - ((char *)g0->arcs)));
        if (i->next)
            i->next = (node *)((char *)nodes +
                               (((char *)i->next) - ((char *)g0->nodes)));
    }

    arc_last = arcs + (g0->arc_last - g0->arcs);
    memcpy(arcs, g0->arcs, (g0->arc_last - g0->arcs) * sizeof(arc));
    for (a = arcs; a < arc_last; a++) {
        a->head = (node *)((char *)nodes +
                           (((char *)a->head) - ((char *)g0->nodes)));
        if (a->next)
            a->next = (arc *)((char *)arcs +
                              (((char *)a->next) - ((char *)g0->arcs)));
        a->sister = (arc *)((char *)arcs +
                            (((char *)a->sister) - ((char *)g0->arcs)));
    }
    ...
    queue_last[0] =
        (g0->queue_last[0] == NULL)
            ? NULL
            : (node *)((char *)nodes +
                       (((char *)g0->queue_last[0]) - ((char *)g0->nodes)));
    queue_last[1] =
        (g0->queue_last[1] == NULL)
            ? NULL
            : (node *)((char *)nodes +
                       (((char *)g0->queue_last[1]) - ((char *)g0->nodes)));
    TIME = g0->TIME;
}
